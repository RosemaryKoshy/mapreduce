...
void MapReduce::MR_Run(int argc, char *argv[], MapReduce::mapper_t map,
    ...
    // loops through files and adds map function to thread queue, if threadqueue
    // is bigger than num_maps pop and then continue.
    ...
    for (int i = 0; i < (int) keys.size(); i++) {
        ...
        threadQueue.emplace(std::async(std::launch::async, reduce, keys[i],
                                       getter, partition(keys[i], num_part)));
    }
